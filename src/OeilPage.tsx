...
const OeilPage: React.FC<OeilPageProps> = ({ onWin, onLose, hardcore }) => {
  ...
  // compteur éventuellement affichable plus tard
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const answerObj = vjlData.find(p => p.id == answerData.personId) || null;
        setAnswer(answerObj);

        // Image de l'oeil du jour: servie par le backend si présente dans public/eyes
        try {
          if (answerObj?.discordUserId) {
            const oeil = await fetchOeilOfTheDay();
            setEyeUrl(oeil.localPath);
          } else {
            setEyeUrl('');
          }
        } catch {
  ...
  const handleGuess = useCallback((person: VJLPerson) => {
    setGuesses(prevGuesses => {
      ...
      if (answer && person.id === answer.id) {
        ...
        setTimeout(async () => {
          if (hardcore && onWin) {
            // En hardcore: pas de sauvegarde/rang/confettis, comme les autres modes
            onWin();
          } else {
            try {
              // Calcule le rang comme sur les autres modes
              const count = await fetchWinnersCount(GAME_MODE);
              setMyRank(count + 1);
  ...
  // Guesses sous forme d'objets
  const guessObjects = useMemo(() => (
    guesses.map(id => vjlData.find(p => p.id == id)).filter(Boolean) as VJLPerson[]
  ), [guesses, vjlData]);
  ...
  // Scroll vers la victoire même après un refresh si on a déjà gagné
  useEffect(() => {
  ...
  // Style de l'image: N/B puis couleur après COLORIZE_STEP essais
  const imgFilter = guesses.length >= COLORIZE_STEP ? 'none' : 'grayscale(1)';
  ...
  if (error) return <div>Erreur de chargement des données : {String(error)}</div>;
      ...
      {(hasWon && showVictoryBox) && (
        <div ref={resultRef} style={{ margin: '32px 0 24px 0' }}>
          <VictoryBox 
            ...
            rank={myRank} 
          />
          <div className="victory-history-box" style={{ margin: '18px auto', maxWidth: 500 }}>
